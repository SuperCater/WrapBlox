...
export default class WrapBlox {
	...
	login = async (cookie: string) => {
		...
		const userInfo = await this.fetchHandler.fetchLegacyAPI(
			"GET",
			"Users",
			"/users/authenticated",
		);
	 ...
	 */
	private fetchRawUser = async (
		query: string | number,
		useCache = true,
	): Promise<RawUserData> => {
		if (typeof query === "number") {
			return await this.fetchHandler.fetchLegacyAPI(
				"GET",
				"Users",
				`/users/${query}`,
				{ useCache: useCache },
	 ...
	 */
	private fetchRawBadge = async (
		badgeId: number,
		useCache = true,
	): Promise<RawBadgeData> => {
		return await this.fetchHandler.fetchLegacyAPI(
			"GET",
			"Badges",
			`/badges/${badgeId}`,
	 ...
	 */
	private fetchRawUniverse = async (
		universeId: number,
		useCache = true,
	): Promise<RawUniverseData> => {
		return (
			await this.fetchHandler.fetchLegacyAPI("GET", "Games", "/games", {
				useCache: useCache,
				params: {
